...
describe('getTileUrl', () => {
  ...
  it('should return the URL for the "osm" basemap', () => {
    ...
    const basemap = 'osm';
    expect(getTileUrl(customUrl, basemap)).toBe('https://tile.openstreetmap.org/{z}/{x}/{y}.png');
  });

  it('should return the URL for the "otm" basemap', () => {
    const customUrl = null; 
    const basemap = 'otm';
    expect(getTileUrl(customUrl, basemap)).toBe('https://tile.opentopomap.org/{z}/{x}/{y}.png');
  });

  it('should return an empty string and log an error for an unsupported basemap', () => {
    const customUrl = null;
    const basemap = 'unsupported';
    const expectedErrorMessage = 'Unsupported basemap: "unsupported"! Use "osm", "topo" or remove the "basemap" parameter.';
