...
describe("Bound Class", () => {
  it("should calculate the correct bounding box", () => {
    ...
    expect(result).toEqual([-120.49280, 36.81084, -118.49280, 38.81084])
  })

  it("should handle a single coordinate", () => {
    const coords: [number, number][] = [[-119.49280, 37.81084]]

    const bound = new Bound({ coords })
    const result = bound.extent()

    expect(result).toEqual([-119.49280, 37.81084, -119.49280, 37.81084])
  })

  it("should throw an error when no coordinates are provided", () => {
    const bound = new Bound({ coords: [] })

    expect(() => bound.extent()).toThrow("Coordinates are required to calculate the bounding box.")
  })
